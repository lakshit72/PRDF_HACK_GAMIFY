...
function FlameCalendar({ current }) {
  const days = Array.from({ length: 14 }, (_, i) => i < current % 14 || (current >= 14 && i < 14));
  return (
    <div className="flex gap-1.5 flex-wrap mt-3">
      ...
      <div className="w-full text-muted text-[10px] font-body mt-1">Last 14 days</div>
    </div>
  );
}
  ...
  return (
    <div className="card relative overflow-hidden">
      ...
      <div className="flex items-end gap-6 mt-3">
        <div>
          <p className="text-muted text-[10px] font-body uppercase tracking-wide mb-1">Current</p>
          <div className="flex items-end gap-2">
            <span className="font-display text-5xl font-extrabold text-text-primary leading-none">{current}</span>
            <span className="text-text-secondary text-sm font-body mb-1">days</span>
          </div>
        </div>
        <div className="pb-1">
          <p className="text-muted text-[10px] font-body uppercase tracking-wide mb-1">Best</p>
          <div className="flex items-end gap-1">
            <span className="font-mono text-2xl font-bold text-gold">{longest}</span>
      ...
      <div className="flex items-start justify-between mb-1.5">
        ...
        <div className="shrink-0 ml-3">
          {quest.completed ? (
            ...
            <span className="text-[10px] font-mono text-muted">{quest.progress}%</span>
          )}
        </div>
      </div>
...
function QuestsSection({ quests }) {
  ...
  return (
    <div className="card">
      ...
      {quests.length === 0 ? (
        <p className="text-muted text-sm font-body text-center py-4">No quests yet</p>
      ) : (
...
const TIER_META = {
  Excellent:    { color: '#6ee7b7', bg: 'rgba(110,231,183,0.12)', border: 'rgba(110,231,183,0.25)' },
  Good:         { color: '#f5c542', bg: 'rgba(245,197,66,0.10)',  border: 'rgba(245,197,66,0.25)'  },
  Fair:         { color: '#ff6b35', bg: 'rgba(255,107,53,0.10)',  border: 'rgba(255,107,53,0.25)'  },
  'Needs Work': { color: '#64748b', bg: 'rgba(100,116,139,0.08)', border: 'rgba(100,116,139,0.20)' },
};
  ...
  return (
    <div className="card">
      ...
      <p className="text-muted text-[10px] font-body text-right mt-4">
        Last calculated · {new Date(score.calculatedAt).toLocaleDateString('en-IN')}
      </p>
    </div>
  );
}
      ...
      {standalone && (
        <>
          ...
          <div className="relative max-w-lg mx-auto px-4 pt-10 pb-28">
            ...
            <GamificationContent streak={streak} score={score} quests={quests} loading={loading} refresh={refresh} />
          </div>
        </>
      )}
